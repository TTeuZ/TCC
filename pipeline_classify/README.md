...
This version of the pipeline aims to recreate the labeling_pipeline flow using a seguence of a posteriori thresholds specified in the config in order to find the best threshold.

The flow is the same as the labeling pipeline, but it runs one time for each threshold specified.
...
- For each subset;
    - for each threshold in the thresholds array:
        - Classify the n initial days with the father ensemble;
            - Select only the ones that the model is more than the actual threshold sure that the label is correct;
